...
# --- Main Dropbox: push project/ (mirror) ---
...
# --- Overleaf: push configured directories (mirror) ---
...
# --- Main Dropbox: pull into project/ (no delete) ---
if [[ -n "$DROPBOX_PATH" ]]; then
...
# --- Overleaf: pull configured directories (no delete) ---
if [[ -n "$OVERLEAF_PATH" && -n "$OVERLEAF_PULL" ]]; then
    ...
    for pair in "${PAIRS[@]}"; do
        ...
        dest="${pair##*:}"
        if [[ "$src" == "." ]]; then
            SRC=$(to_win "$OVERLEAF_PATH")
        else
            SRC=$(to_win "$OVERLEAF_PATH/$src")
        fi
...
if [[ -n "$DROPBOX_PATH" ]]; then
    ...
    echo "  push: every commit syncs project/ to Dropbox (mirror)"
